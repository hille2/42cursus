...
/*   Updated: 2021/02/07 15:07:14 by sgath            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_rez_line(char **line, char **rem_str, int ret)
{
	...
	if ((n_str < ft_strlen_gnl(*rem_str) && n_str >= 0) && ret >= 0)
	{
		...
		while (++i < n_str)
			(*line)[i] = (*rem_str)[i];
		(*line)[n_str] = '\0';
		if (!(*rem_str = ft_substr_gnl(*rem_str, n_str + 1,
		ft_strlen_gnl(*rem_str))))
			return (-1);
		return (1);
	}
...
int			get_next_line(int fd, char **line)
{
	...
	while (count == 0 && (ret = read(fd, buf, BUFFER_SIZE)))
	{
		...
		(!rem_str) ? rem_str = ft_strdup(buf) :
			(rem_str = ft_strjoin_gnl(rem_str, buf));
		if (ft_strchr(buf, '\n'))
			count = 1;
	}
	if (ret == 0 && !rem_str)
