...
/*   Updated: 2021/07/17 22:14:56 by yu               ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_one_philo(t_all *all)
{
	...
	while (++i < all->num_of_philo)
	{
		...
		if (i > i % (all->num_of_philo - 1))
		{
			all->one[i].largest_fork = &all->forks[i];
			all->one[i].smallest_fork = &all->forks[(i + 1) % (all->num_of_philo)];
		}
		else
		{
			all->one[i].smallest_fork = &all->forks[i + 1];
			all->one[i].largest_fork = &all->forks[i % all->num_of_philo];
		}
		all->one->time_die = all->time_to_die;
		all->one[i].all = all;
	}
}
...
void	init_option(char **av, t_all *all)
{
	...
	if (all->num_of_philo == 0 || all->num_of_philo == MAX_SIZE || all->num_of_philo > MAX_PHILO)
